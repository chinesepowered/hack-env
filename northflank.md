...
```bash
northflank exec service --projectId hackathon --serviceId red-team
```
...
Persistent disk is mounted at `/persist/` — venv, repo, and model weights survive reboots. `/tmp/` and `/app/` do not.
...
That's it — venv, repo, and model weights all persist. Run scripts directly from `/persist/repo/training/`.
...
## Run Training (H100 — full script)

```bash
TORCHDYNAMO_DISABLE=1 /persist/trainvenv/bin/python /persist/repo/training/train.py \
  --model Qwen/Qwen3-4B \
  --output-dir /persist/output/angry_claw
```
...
## Run Training (Colab script — smoke test / notebook parity check)

```bash
TORCHDYNAMO_DISABLE=1 /persist/trainvenv/bin/python /persist/repo/training/train_colab.py \
  --output-dir /persist/output/angry_claw_colab
```

Uses `load_in_4bit=True` and smaller defaults (dataset-size 32). Good for verifying the Colab notebook runs the same end-to-end on the H100.
...
```bash
TORCHDYNAMO_DISABLE=1 /persist/trainvenv/bin/python /persist/repo/training/eval_model.py \
  --model Qwen/Qwen3-4B \
  --lora-path /persist/output/angry_claw/final \
  --episodes 10
```
...
## Start the Environment Server (optional)

The HTTP server is only needed if using `EnvHTTPClient`. Training uses `DirectEnvClient` (in-process) so the server is **not required**.
...
| `--output-dir` | `./output/angry_claw` | Use `/persist/output/...` so weights survive reboot |
...
LoRA adapters saved to `--output-dir/final/`:
```bash
ls /persist/output/angry_claw/final/
```
